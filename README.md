...
The action implementation is generated by appending the statement for invoking the parameterized API (i.e., **cAPI(params)**) of the corresponding action.
...
Among the set of actions, we design a pick-and-place task.
...
The actions of a ***Fetch*** used in the case study are listed in table below.

<div align="center">
    <p>Table: Actions of Fetch used in the case study</p>
</div>
...
The execution of the actions in the simulation environment shows uncertainty due to the physics engine or the probability-based algorithms. For example, an object slips away from the gripper of a robot, or a robot follows different moving paths by the SLAM navigation algorithm.
...
#### Offline Training
We train the ***capNet*** based on the data set obtained from executing the four actions randomly.
...
We execute each action more than one thousand times and obtain the data set containing more than 4,000 trajectories.
...
<div align="center">
    <tr>⟨(move<sub>1</sub>, move, prop<sub>1</sub>),</tr>
...
#### Testing
We test the generated robotic program by proposing a new request but under a slightly changed environment setting.
...
The request is realized by customizing the parameters of the action APIs, and thus the actions can be executed as expected.
...
<!-- 
The raw data of the evaluation is provided in [folder](https://github.com/LCEPRPFD/LCEPRPFD.github.io/tree/main/data). -->
...
<div align="center">
    <p>Table: Accuracy of proposition verdict of different approaches</p>
</div>
...
<div align="center">
    <p>Table: Accuracy of evaluating propositions for SeR under different demonstration counts</p>
</div>
...
<div align="center">
    <p>Table: Accuracy of evaluating propositions reused in the eight-step task</p>
</div>
...
We retain the networks learned in the four-step task by 5 demonstrations.
Then, an eight-step Π<sub>task</sub> can be assembled as
<div align="center">
    <tr>⟨(move<sub>1</sub>, move, prop<sub>1</sub>),</tr>
    <tr>(pick<sub>1</sub>, pick, prop<sub>2</sub>), </tr>
    <tr>(transport<sub>1</sub>, transport, prop<sub>3</sub>), </tr>
...
The task is executed for 50 times in _SeR_ setting.
The accuracy for evaluating propositions is listed in Table.
